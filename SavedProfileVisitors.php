...
<section class="content greybg listing">
 ...
 <?php } ?>
      <h2 class="modren_title">Saved Profiles</h2>
...
return $years = ceil($diff / (365*60*60*24));
	...
	$path = "SavedProfileVisitors.php";
	$condition ="";
	$tab = "";
							
			 $sqlr = "select * from `saved` where `uid` = $uid";
			 $resultr = mysql_query($sqlr);
			 $tr = mysql_num_rows($resultr);
			 if($tr > 0) {
				 $sqlr = "select * from `saved` where `uid` = $uid order by `entry_id` desc LIMIT $limitvalue, $limit";
			 $resultr = mysql_query($sqlr);
			while($resr = mysql_fetch_object($resultr)) { 
			$sqls = "select your_own_words,state,working_as,name,code,entry_id,dob,height,religion,community,city from `profiles` where `entry_id` = $resr->sid and `gender` != '$res_user->gender'";
...
<?php } else { ?>
<h3 align="center">There are no saved profiles found for this account</h3>
<?php } ?>
      </div>
...
<?php require_once'includes/footer.php'; ?>
  ...
  <script language="javascript">
	  function deleted(id)
	  {
		...
		{
		 document.location.href='SavedProfileVisitors.php?deleteid='+id;
		}
	  }
	 </script>
	   <?php
	  ...
	  $uid = $_SESSION['uid'];
	  $sql = "delete from `saved` where `uid` = $uid and `entry_id` = $did";
	  $result = mysql_query($sql);
	   if($result)
	  {
	   echo "<script>document.location.href='SavedProfileVisitors.php?msg=delete';</script>";
