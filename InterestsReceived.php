...
<section class="content greybg listing">
 ...
 <?php } ?>
      <h2 class="modren_title">Interests Received</h2>
...
return $years = ceil($diff / (365*60*60*24));
	...
	$path = "InterestsReceived.php";
	$condition ="";
	$tab = "";
							
			 $sqlr = "select * from `express` where `eid` = $uid and `eid_delete_status` = 0";
			 $resultr = mysql_query($sqlr);
			 $tr = mysql_num_rows($resultr);
			 if($tr > 0) {
				 $sqlr = "select * from `express` where `eid` = $uid and `eid_delete_status` = 0 order by `entry_id` desc LIMIT $limitvalue, $limit";
			 $resultr = mysql_query($sqlr);
...
<?php } else { ?>
<h3 align="center">There are no interests received profiles found for this account</h3>
<?php } ?>
      </div>
...
<?php require_once'includes/footer.php'; ?>
  ...
  <script language="javascript">
	  function deleted(id)
	  {
		...
		{
		 document.location.href='InterestsReceived.php?deleteid='+id;
		}
	  }
	 </script>
	   <?php
	  ...
	  $uid = $_SESSION['uid'];
	  $sql = "update `express` set `eid_delete_status` = '1' where `eid` = $uid and `entry_id` = $did";
	  $result = mysql_query($sql);
	   if($result)
	  {
	   echo "<script>document.location.href='InterestsReceived.php?msg=delete';</script>";
